...
export const HomeScreen: React.FC<{ onOpenEditBudget: () => void }> = ({
  ...
  // Selected date for day-by-day navigation & backdating
  ...
  const selectedDateFormatted = useMemo(() => {
    ...
    if (selectedDate === yesterdayDateString) return 'Yesterday';
    try {
  ...
  }, [selectedDate, todayDateString, yesterdayDateString]);

  // Expenses filtered for the currently viewed date
  const displayedExpenses = useMemo(() => {
    return expenses
      .filter((e) => !e.isDelayed && e.dateString === selectedDate)
  ...
  const handleAddExpense = (
    ...
    addExpense(amount, description, timeFormatted, category, dateString || selectedDate);
    setLastEmojiAction({
      type: 'ADD',
  ...
  const handleAddMoney = (amount: number, description?: string, dateString?: string) => {
    addAdditionalMoney(amount, description, dateString || selectedDate);
    setLastEmojiAction({
      type: 'CELEBRATION',
      dailyAllowance: todayAllowance,
      timestamp: Date.now(),
  ...
  return (
    <div className="min-h-screen min-h-[100dvh] w-full bg-[#FBFBFE] dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors pb-[calc(env(safe-area-inset-bottom,0px)+7.5rem)]">
      ...
      >
        ...
        {/* Day Navigation Bar (Previous / Next Day) */}
        <motion.div
          variants={{
            hidden: { opacity: 0, y: 8 },
            visible: {
          ...
          className="flex items-center justify-between bg-white dark:bg-slate-900 border border-indigo-100/90 dark:border-slate-800 rounded-2xl px-3 py-2 shadow-2xs"
        >
          <button
            onClick={handlePrevDay}
            className="p-1.5 rounded-xl text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition cursor-pointer flex items-center gap-1 text-xs font-semibold"
          ...
          <div className="flex items-center gap-2">
            ...
            {!isViewingToday && (
              <button
                ...
                className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800 hover:bg-indigo-100 transition cursor-pointer"
              >
                Today
              </button>
            )}
          ...
          <button
            ...
            className="p-1.5 rounded-xl text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 disabled:opacity-30 disabled:cursor-not-allowed transition cursor-pointer flex items-center gap-1 text-xs font-semibold"
            aria-label="Next Day"
          >
        ...
        >
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-[#1E1B4B] dark:text-white">
                {isViewingToday ? "Today's Spending" : `${selectedDateFormatted}'s Overview`}
              </span>
            </div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 text-center font-normal pt-0.5">
            &ldquo;
            {spendingMood.headline ||
              spendingMood.message ||
        ...
        {/* 3. Prominent Action Buttons: "+ Add Expense" & "+ Add Money" */}
        <motion.div
          variants={{
            hidden: { opacity: 0, y: 8 },
            visible: {
        ...
        >
          <motion.button
            ...
            id="home-add-expense-button"
            onClick={() => {
              setShowAddSheet(true);
            }}
            className="w-full py-3.5 px-3 rounded-2xl bg-[#120E3D] hover:bg-[#1a1554] dark:bg-indigo-600 dark:hover:bg-indigo-500 text-white font-bold text-xs sm:text-sm shadow-md transition flex items-center justify-center gap-1.5 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-slate-950"
          >
          ...
          <motion.button
            whileTap={{ scale: 0.97 }}
            transition={{ duration: 0.12 }}
            id="home-add-money-button"
            onClick={() => {
              setShowAddMoneySheet(true);
            }}
            className="w-full py-3.5 px-3 rounded-2xl bg-emerald-700 hover:bg-emerald-600 dark:bg-emerald-600 dark:hover:bg-emerald-500 text-white font-bold text-xs sm:text-sm shadow-md transition flex items-center justify-center gap-1.5 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-slate-950"
          >
            <Plus className="w-4 h-4" />
            <span>Add Money</span>
          </motion.button>
        ...
        >
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-[#1E1B4B] dark:text-white">
              {isViewingToday ? "Today's Expenses" : `${selectedDateFormatted}'s Expenses`}
            </h3>
            ...
            {displayedExpenses.length > 0 && (
              ...
              >
                ...
                <span>{displayedExpenses.length} logged</span>
              </motion.button>
            )}
          </div>
            ...
            <div className="py-12 px-4 rounded-3xl bg-transparent text-center space-y-1">
              <p className="text-xs font-bold text-slate-700 dark:text-slate-300">
                {isViewingToday
                  ...
                  : `No expenses recorded for ${selectedDateFormatted}.`}
              </p>
              <p className="text-xs text-slate-400 dark:text-slate-500">
                {isViewingToday
                  ? 'Tap + Add Expense above to record your first spend.'
                  : 'Tap + Add Expense above to backdate an expense to this day.'}
              </p>
            </div>
          ) : (
      ...
      <AddExpenseSheet
        ...
        defaultDateString={selectedDate}
      />

      <AddMoneySheet
        currencySymbol={currencySymbol}
        isOpen={showAddMoneySheet}
